...
class crudEdit extends Command
{
     ...
     * @var string
     */
    protected $signature = 'crud:edit {model}';
     ...
     * @var string
     */
    protected $description = 'Create a view Edit with crud system (Eg. > php artisan crud:edit Todo)';
    ...
    public function handle()
    {
        ...
        foreach($std as $k => $v){
            ...
            if($pos !== false){
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="'.$v->Field.'" class="form-control @error(\''.$v->Field.'\') is-invalid @enderror" value="{{ $'.$model_lower.'->'.$v->Field.' }}" id="input'.$v->Field.'" aria-describedby="'.$v->Field.'Help">
                    @error("'.$v->Field.'")
        ...
        $contenuMod=str_replace(
            ['_table_', '_form_group_', '_model_lower_'],
            [$table, implode('',$form_group), $model_lower],
            $contenu
        );
        fclose($text);
